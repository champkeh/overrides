...
const __TENCENT_CHAOS_VM = function() {
  let g = function A(initial, count, step) {
    let result = []
    let i = 0
    while (i++ < count) {
      result.push(initial += step)
    }
    return result
  };
  let Q = [
  ...
  let E = function A(A) {
    ...
    let base64str = String(A).replace(/[=]+$/, ""),
        len = base64str.length,
        Y, I, C = 0, o = [];

    for (let i = 0; i < len; i++) {
      I = Q[base64str.charCodeAt(i)];
      ~I && (Y = C % 4 ? 64 * Y + I : I, C++ % 4)
          ? o.push(255 & Y >> (-2 * C & 6))
          : 0
    }
    return o
